...
// "16 * 16 * 4" ifadesi, 16x16 boyutlarında bir matrisin elemanlarının boyutunu temsil eder.
// Bu matrisin elemanları tamsayı (int)
// türünde olduğu için her bir eleman 4 bayt yer kaplar (genellikle bir tamsayı 4 bayt yer işgal eder).
...
__global__ void tiledMatrixMul(int *a, int *b, int *c, int n, int tile_size) {
	...
	__shared__ int A[SHMEM_SIZE]; // 16*16 matris
	...
	for (int i = 0; i < (n / tile_size); i++) {
		/*
            ...
            Anlamaya calıstıgım kısım
            Matrıslerı cache bellege atıyoruz burda onda sıkınıt yok ama nasıl atıyoruz
			For A:
